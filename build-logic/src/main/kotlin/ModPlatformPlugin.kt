...
abstract class ModPlatformPlugin @Inject constructor() : Plugin<Project> {
	...
	private fun Project.configureProcessResources(
		...
		tasks.named<ProcessResources>("processResources") {
			...
			val atResourcePath = "aw/${stonecutter.current.version}.cfg"
			...
			when {
				...
				isNeoForge -> {
					...
					filesMatching("aw/*.cfg") {
						if (path == atResourcePath) path = "META-INF/accesstransformer.cfg" else exclude()
					}
					if (usesLegacyToml) {
	...
	private fun Project.configurePublishing(
		...
		extensions.configure<ModPublishExtension>("publishMods") {
			...
			displayName = if (fullDisplayName.length > 64) {
				fullDisplayName.replace("Enhanced Block Entities", "EBE").take(64).trim()
			} else {
				fullDisplayName
			}
