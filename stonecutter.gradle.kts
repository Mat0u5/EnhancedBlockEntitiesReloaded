...
publishMods {
	...
	if (shouldAnnounceDiscord) {
		...
		discord("publishDiscordBody") {
			...
			if (!isDev) {
				content = changelog.map { "# [EnhancedBlockEntitiesReloaded version `$version` is out!](https://modrinth.com/mod/enhancedblockentities/versions)\n" +
					"### Changelog:\n" +
					"```\n$it```\n"  }
			}
			else {
				content = changelog.map { "# [EnhancedBlockEntitiesReloaded version `$version` is out!](https://modrinth.com/mod/enhancedblockentities-dev/versions)\n" +
					"### Changelog:\n" +
					"```\n$it```\n" }
			}
